...
namespace NullSave.GDTK
{
    public class Simple3rdPersonCamera : MonoBehaviour
    {
        ...
        private ClipPlanePoints NearClipPlanePoints(Vector3 pos, float clipPlaneMargin)
        {
            ...
            var transform = useCamera.transform;
            var halfFOV = (useCamera.fieldOfView / 2) * Mathf.Deg2Rad;
            var aspect = useCamera.aspect;
            var distance = useCamera.nearClipPlane;
        ...
        private void UpdateInput()
        {
            ...
            float x = InterfaceManager.Input.GetAxis(inputHoriz);
            float y = InterfaceManager.Input.GetAxis(inputVert);
        ...
        private void UpdateMovement()
        {
            ...
            if (CullingRayCast(current_cPos, planePoints, out _, distance, cullingLayer, Color.cyan)) distance = Mathf.Clamp(cullingDistance, 0.0f, defaultDistance);
