...
{
    public class SAFPluginPicker : PopupWindowContent
    {

        #region Fields

        public SerializedProperty targetList;
        public List<SAFPluginWrapper> target;
        public string listType;
        public float width;

        private List<UniversalPluginInfo<SortAndFilterPlugin>> plugins;
        private List<UniversalPluginInfo<SortAndFilterPlugin>> filteredPlugins;
        ...
        public override void OnGUI(Rect rect)
        {
            if (plugins == null)
            {
                ...
                plugins = new List<UniversalPluginInfo<SortAndFilterPlugin>>();
                plugins.AddRange(UniversalPluginManager<SortAndFilterPlugin>.Plugins.Where(x => x.plugin.compatibleListTypes.Contains(listType)));
                filteredPlugins = new List<UniversalPluginInfo<SortAndFilterPlugin>>();
                    ...
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
                    {
                        if (lastDownIndex == i && TimeSpan.FromTicks(DateTime.Now.Ticks - lastDownTicks) <= TimeSpan.FromSeconds(0.25f))
                        {
                            if (target != null)
                            {
                                ...
                                target.Add(new SAFPluginWrapper((SortAndFilterPlugin)Activator.CreateInstance(filteredPlugins[i].plugin.GetType())));
                                if (targetList != null)
                                {
