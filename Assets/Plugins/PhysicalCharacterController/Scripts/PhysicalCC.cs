...
public class PhysicalCC : MonoBehaviourPun
{
    ...
    private void Update()
    {
        ...
        if (isGround)
        {
            ...
            if (inertiaVelocity != Vector3.zero) InertiaDamping();
        }
    ...
    private void InertiaDamping()
    {
        ...
        inertiaVelocity = Vector3.Lerp(inertiaVelocity, a, inertiaDampingTime * Time.deltaTime);
        // inertiaVelocity = (inertiaVelocity + moveVelocity).magnitude <= 0.5f ? Vector3.zero : Vector3.SmoothDamp(inertiaVelocity, Vector3.zero, ref a, inertiaDampingTime / (3 / (180 / resistanceAngle)));
    }

    private void GroundCheck()
    {
        var cols = Physics.OverlapSphere(transform.position, cc.radius, groundMask);

        if (cols.Length > 0)
        {
            isGround = true;
            if (Physics.SphereCast(transform.position, cc.radius, Vector3.down, out RaycastHit hit, groundCheckDist, groundMask))
            {
