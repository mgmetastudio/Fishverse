...
{
    public class StatPluginPicker : PopupWindowContent
    {

        #region Fields

        public SerializedProperty targetList;
        public List<UniversalPluginWrapper<StatsPlugin>> target;
        public float width;

        private List<UniversalPluginInfo<StatsPlugin>> plugins;
        private List<UniversalPluginInfo<StatsPlugin>> filteredPlugins;
        ...
        public override void OnGUI(Rect rect)
        {
            if (plugins == null)
            {
                ...
                plugins = UniversalPluginManager<StatsPlugin>.Plugins;
                filteredPlugins = new List<UniversalPluginInfo<StatsPlugin>>();
                    ...
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
                    {
                        if (lastDownIndex == i && TimeSpan.FromTicks(DateTime.Now.Ticks - lastDownTicks) <= TimeSpan.FromSeconds(0.25f))
                        {
                            if (target != null)
                            {
                                ...
                                target.Add(new UniversalPluginWrapper<StatsPlugin>((StatsPlugin)Activator.CreateInstance(filteredPlugins[i].plugin.GetType())));
                                if (targetList != null)
                                {
